...
func APIKeyAuth(keySvc KeyLookup, tenantSvc TenantStatusGetter) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if auth == "" {
			WriteError(c, http.StatusUnauthorized, "missing API key", "auth_error")
			return
		}

		rawKey := extractBearer(auth)
		if rawKey == "" || !strings.HasPrefix(rawKey, "sk-agw-") || len(rawKey) < 39 {
			WriteError(c, http.StatusUnauthorized, "invalid API key format", "auth_error")
			return
		}
		...
		if err != nil {
			WriteError(c, http.StatusUnauthorized, "invalid API key", "auth_error")
			return
		}
		...
		if key.ExpiresAt != nil && time.Now().After(*key.ExpiresAt) {
			WriteError(c, http.StatusUnauthorized, "API key expired", "auth_error")
			return
		}
		...
		if tenant.Status == model.TenantStatusDisabled {
			WriteError(c, http.StatusForbidden, "tenant disabled", "forbidden")
			return
		}
